...
def _get_cur_date_hour(request_from: str = ''):
    cur_dt = datetime.now()
...
def _check_currentness_query_params(resort: str, request_from: str = ''):
    cur_dt = _get_cur_date_hour(request_from=request_from)
    return {
        'resort': resort,
        'date': cur_dt[0],
        'hour': cur_dt[1]
...
class BaseWeatherDb(BaseDb):
    ...
    def _check_weather_is_current(self, resort: str):
        query_params = _check_currentness_query_params(resort, request_from='weather')
...
class WeatherDbReader(BaseWeatherDb):
    ...
    def get_weather_info(self, resort: str):
        cur_dt = _get_cur_date_hour(request_from='weather')
...
class BaseTravelDb(BaseDb):
    ...
    def _check_driving_is_current(self, resort: str):
        query_params = _check_currentness_query_params(resort, request_from='driving')
        self.cursor.execute(check_driving_resort_current, query_params)
        return self._check_currentness_result()

    def _check_flying_is_current(self, resort: str):
        query_params = _check_currentness_query_params(resort, request_from='flying')
...
class TravelDbReader(BaseTravelDb):
    ...
    def get_travel_info(self, resort: str):
        if get_resort_driving(resort):
            cur_dt = _get_cur_date_hour(request_from='driving')
            return {
                'mode': 'driving',
                'info': self._get_cur_driving_info(resort, cur_dt)
            }
        cur_dt = _get_cur_date_hour(request_from='flying')
        return {
            'mode': 'flying',
...
class TravelDbBackgroundProcess(BaseTravelDb):
    ...
    def add_drive_info(self, drive_time_info: dict, resort: str):
        ...
        cur_dt_hour = _get_cur_date_hour(request_from='driving')
        drive_tup = (
            resort,
            cur_dt_hour[0],
            cur_dt_hour[1],
